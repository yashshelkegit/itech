import img1 from "../event-pics/1.jpg";
import img2 from "../event-pics/2.jpg";
import img3 from "../event-pics/3.jpg";
import img4 from "../event-pics/4.jpg";
import img5 from "../event-pics/5.jpg";
import img6 from "../event-pics/6.jpg";
import img7 from "../event-pics/7.jpg";
import img8 from "../event-pics/8.jpg";
import img9 from "../event-pics/9.jpg";
import img10 from "../event-pics/10.jpg";
import img11 from "../event-pics/11.png";
import img12 from "../event-pics/12.jpg";
import img13 from "../event-pics/13.jpg";
import img14 from "../event-pics/14.jpg";
import img15 from "../event-pics/15.jpg";


const eventData = [
	{
		id: 1,
		title: "Hackathon",
		description:
			"Dive into a fast-paced hackathon: team up, innovate, and showcase your solutions in a thrilling, time-limited challenge.",
		image: img1,
		formLink: "https://forms.gle/75fwPcQDXLBmqUFp6",
		rulebook:
			"https://drive.google.com/file/d/1y8fbqTRZs9yOFSoX3hytvh0PLRi66LpW/view?usp=sharing",
		category: "upcoming",
		price: {
			pool: "Upto 15k and UnStop Merch (T-shirt) to winners and finalist",
			first: "10,000",
			second: "5,000",
		},
		entryFees: "Rs 100/- per member",
		memberCount: "Min 3 and Max 5 members",
		date: "23/08/2024",
		organizers: [
			{
				name: "Rehan Khan",
				email: "info.rehan@gmail.com",
				phone: "+91 8208948815",
			},
			{
				name: "Hitanshu Dadhich",
				email: "info.hitanshu@gmail.com",
				phone: "+91 8949122605",
			},
		],
		coOrganizers: [
			{
				name: "Rohan Bendale",
				email: "info.rohan@gmail.com",
				phone: "+91 XXXXX XXXXX",
			},
			{
				name: "Yash Shelke",
				email: "info.yash@gmail.com",
				phone: "+91 XXXXX XXXXX",
			},
		],
	},
	{
		id: 2,
		title: "Fix-a-thon",
		description:
			"Join the Fixathon and solve intricate problems individually. Challenge yourself and refine your skills.",
		image: img2,
		formLink: "https://forms.gle/cd237k3iLPuK2r9g6",
		rulebook: "",
		category: "upcoming",
		price: { pool: "Upto 1.5k", first: "1,000", second: "500" },
		entryFees: "70/-",
		memberCount: "individual",
		date: "24/08/2024",
		organizers: [
			{
				name: "Abhishek Bhoyar",
				email: "info.abhishek@gmail.com",
				phone: "+91 7709869270",
			},
			{
				name: "Sahil Ninawe",
				email: "info.sahil@gmail.com",
				phone: "+91 8767993086",
			},
		],
		coOrganizers: [
			{
				name: "Somesh Sharma",
				email: "info.somesh@gmail.com",
				phone: "+91 XXXXX XXXXX",
			},
			{
				name: "Sanchit Yelne",
				email: "info.sanchit@gmail.com",
				phone: "+91 XXXXX XXXXX",
			},
		],
	},
	{
		id: 3,
		title: "Web Design Jam",
		description:
			"Show off your web design skills in the Web Design Jam. Create a stunning website in limited time!",
		image: img15,
		formLink:
			"https://docs.google.com/forms/d/e/1FAIpQLScdhwlfNe5nkO3SCk5I9OsiwBQd_M4ICZiRunDpv0SYx4D2Ww/viewform",
		rulebook: "",
		category: "upcoming",
		price: { pool: "Upto 1k", first: "1,000", second: "NA" },
		entryFees: "70/- ",
		memberCount: "individual",
		date: "24/08/2024",
		organizers: [
			{
				name: "Pralendra Behera",
				email: "info.pralendra@gmail.com",
				phone: "+91 9322560971",
			},
			{
				name: "Abdul Aziz Aman",
				email: "info.abdul@gmail.com",
				phone: "+91 8600073906",
			},
		],
		coOrganizers: [
			{
				name: "Shruti Kathale",
				email: "info.shruti@gmail.com",
				phone: "+91 XXXXX XXXXX",
			},
			{
				name: "Kusum Hari",
				email: "info.kusum@gmail.com",
				phone: "+91 XXXXX XXXXX",
			},
		],
	},
	{
		id: 4,
		title: "Quiz Mania",
		description:
			"Test your knowledge in a series of challenging quizzes. Compete solo and see how much you know!",
		image: img3,
		formLink: "https://forms.gle/f8ZiNLwv2aEECKeH9",
		rulebook: "",
		category: "upcoming",
		price: { pool: "Upto 1k", first: "1,000", second: "NA" },
		entryFees: "60/-",
		memberCount: "individual",
		date: "24/08/2024  (online)",
		organizers: [
			{
				name: "Chaitali Nagbhidkar",
				email: "info.chaitali@gmail.com",
				phone: "+91 9307984568",
			},
			{
				name: "Trisha Kasumbiwal",
				email: "info.trisha@gmail.com",
				phone: "+91 9359555184",
			},
		],
		coOrganizers: [
			{
				name: "Nidhi Nilewar",
				email: "info.nidhi@gmail.com",
				phone: "+91 XXXXX XXXXX",
			},
			{
				name: "Taniya Borkar",
				email: "info.taniya@gmail.com",
				phone: "+91 XXXXX XXXXX",
			},
		],
	},
	{
		id: 5,
		title: "Poster Designing",
		description:
			"Unleash your creativity in the Poster Designing event. Create stunning visuals that speak volumes.",
		image: img4,
		formLink:
			"https://docs.google.com/forms/d/1lWibHLZl_LMM4OZ9SlxgSkKvkGneO9pbT94S9IciWW4/edit",
		rulebook: "",
		category: "upcoming",
		price: { pool: "Upto 1k", first: "1,000", second: "NA" },
		entryFees: "70/-",
		memberCount: "individual",
		date: "23/08/2024",
		organizers: [
			{
				name: "Anurag Gaddamwar",
				email: "info.anurag@gmail.com",
				phone: "+91 8080172824",
			},
			{
				name: "Tejas Punwatkar",
				email: "info.tejas@gmail.com",
				phone: "+91 8999517056",
			},
		],
		coOrganizers: [
			{
				name: "Raunak Kalamkhede",
				email: "info.raunak@gmail.com",
				phone: "+91 XXXXX XXXXX",
			},
			{
				name: "Aditya Raut",
				email: "info.aditya@gmail.com",
				phone: "+91 XXXXX XXXXX",
			},
		],
	},
	{
		id: 6,
		title: "Photo Arena",
		description:
			"Capture the moment in the Photo Arena event. Show your photography skills and compete for the best shot.",
		image: img5,
		formLink: "https://forms.gle/A5DwEsWyZhLu6236A",
		rulebook: "",
		category: "upcoming",
		price: { pool: "Upto 700", first: "700", second: "NA" },
		entryFees: "50/-",
		memberCount: "individual",
		date: "24/08/2024",
		organizers: [
			{
				name: "Varsha Bramhankar",
				email: "info.varsha@gmail.com",
				phone: "+91 8080946005",
			},
			{
				name: "Manaswi Padole",
				email: "info.manaswi@gmail.com",
				phone: "+91 7262887773",
			},
		],
		coOrganizers: [
			{
				name: "Riddhi Dongarwar",
				email: "info.riddhi@gmail.com",
				phone: "+91 XXXXX XXXXX",
			},
			{
				name: "Khushi Lakhe",
				email: "info.khushi@gmail.com",
				phone: "+91 XXXXX XXXXX",
			},
		],
	},
	{
		id: 7,
		title: "Box Cricket",
		description:
			"Get your team together and compete in the Box Cricket event. It's fast, fun, and full of action!",
		image: img6,
		formLink: "https://forms.gle/aKYEhFeNCDwnLfCC7",
		rulebook: "",
		category: "upcoming",
		price: {
			pool: "Upto 5k",
			first: "3,500",
			second: "Goodies (worth upto 2k)",
		},
		entryFees: "700/- per group",
		memberCount: "Team of 7",
		date: "23/08/2024 and 24/08/2024",
		organizers: [
			{
				name: "Abhishek Ghule",
				email: "info.abhishekghule@gmail.com",
				phone: "+91 7057530377",
			},
			{
				name: "Kartik Pohekar",
				email: "info.kartik@gmail.com",
				phone: "+91 9766376251",
			},
			{
				name: "Harsh Joshi",
				email: "info.harsh@gmail.com",
				phone: "+91 8767242867",
			},
		],
		coOrganizers: [
			{
				name: "Priyanshu Dange",
				email: "info.priyanshu@gmail.com",
				phone: "+91 XXXXX XXXXX",
			},
			{
				name: "Rohit Gautam",
				email: "info.rohit@gmail.com",
				phone: "+91 XXXXX XXXXX",
			},
			{
				name: "Gaurav Rakde",
				email: "info.gaurav@gmail.com",
				phone: "+91 XXXXX XXXXX",
			},
			{
				name: "Hardik Thakre",
				email: "info.hardik@gmail.com",
				phone: "+91 XXXXX XXXXX",
			},
		],
	},
	{
		id: 8,
		title: "Futsal",
		description:
			"Compete in a fast-paced Futsal tournament. Show your skills and teamwork in this exciting event.",
		image: img7,
		formLink: "https://forms.gle/38E8g8VJd99cWQg27",
		rulebook: "",
		category: "upcoming",
		price: { pool: "Upto 4k", first: "2,500", second: "NA" },
		entryFees: "600/- per group",
		memberCount: "Team of 6",
		date: "23/08/2024 and 24/08/2024",
		organizers: [
			{
				name: "Harshal Dhengale",
				email: "info.harshal@gmail.com",
				phone: "+91 9022734703",
			},
			{
				name: "Gaurav Kalbhut",
				email: "info.gaurav@gmail.com",
				phone: "+91 9529850206",
			},
			{
				name: "Prince Yadav",
				email: "info.gaurav@gmail.com",
				phone: "+91 9112445714",
			},
		],
		coOrganizers: [
			{
				name: "Advait Kadoo",
				email: "info.advait@gmail.com",
				phone: "+91 XXXXX XXXXX",
			},
			{
				name: "Ayush Chandekar",
				email: "info.ayush@gmail.com",
				phone: "+91 XXXXX XXXXX",
			},
		],
	},
	{
		id: 9,
		title: "Chess",
		description:
			"Show your strategic prowess in the Chess tournament. Compete with or without a board.",
		image: img8,
		formLink: "https://forms.gle/CWZVZvC1GYWKMKFFA",
		rulebook: "",
		category: "upcoming",
		price: { pool: "Upto 1.5k", first: "1,000", second: "500" },
		entryFees: "60/-",
		memberCount: "individual",
		date: "23/08/2024",
		organizers: [
			{
				name: "Chiranjeet Roy",
				email: "info.chiranjeet@gmail.com",
				phone: "+91 7498765517",
			},
			{
				name: "Prajat Jibhe",
				email: "info.prajat@gmail.com",
				phone: "+91 8432800636",
			},
		],
		coOrganizers: [
			{
				name: "Labh Goyal",
				email: "",
				phone: "",
			},
			{
				name: "Nandini Joshi",
				email: "",
				phone: "",
			},
			{
				name: "Harsh Jadhav",
				email: "",
				phone: "",
			},
			{
				name: "Harsh Ramteke",
				email: "",
				phone: "",
			},
		],
	},
	{
		id: 10,
		title: "Pirates Map (Treasure Hunt)",
		description:
			"Join the ultimate adventure in Pirates Map. Form a team, follow the clues, and find the treasure!",
		image: img9,
		formLink: "https://forms.gle/dgyrvmJ35wBVBEFR9",
		rulebook: "",
		category: "upcoming",
		price: { pool: "Upto 1.2k", first: "1,200", second: "NA" },
		entryFees: "240/- per group",
		memberCount: "Min 3 and Max 4 members",
		date: "25/08/2024",
		organizers: [
			{
				name: "Aditya Hande",
				email: "info.aditya@gmail.com",
				phone: "+91 8766801667",
			},
			{
				name: "Shivkumar Magar",
				email: "info.shivkumar@gmail.com",
				phone: "+91 9307489625",
			},
		],
		coOrganizers: [
			{
				name: "Aryan Gaikwad",
				email: "info.aaryan@gmail.com",
				phone: "+91 XXXXX XXXXX",
			},
		],
	},
	{
		id: 11,
		title: "E-Sports (BGMI)",
		description:
			"Form your squad and dominate the virtual battlefield in the BGMI E-Sports tournament.",
		image: img10,
		formLink: "https://forms.gle/UwDuJjePkFaG7rsK7",
		rulebook: "",
		category: "upcoming",
		price: { pool: "upto 6k", first: "NA", second: "NA" },
		entryFees: "250/- per group",
		memberCount: "Team of 4",
		date: "23/08/2024",
		organizers: [
			{
				name: "Tanay Ajankar",
				email: "info.tanay@gmail.com",
				phone: "+91 9146375066",
			},
			{
				name: "Satyam Mahajan",
				email: "info.satyam@gmail.com",
				phone: "+91 9309721287",
			},
			{
				name: "Sarthak Tupkar",
				email: "info.sarthak@gmail.com",
				phone: "+91 7755999877",
			},
		],
		coOrganizers: [
			{
				name: "Kedar Poul",
				email: "info.kedar@gmail.com",
				phone: "+91 XXXXX XXXXX",
			},
			{
				name: "Aarsh Nimje",
				email: "info.aarsh@gmail.com",
				phone: "+91 XXXXX XXXXX",
			},
		],
	},
	{
		id: 12,
		title: "Beat the Mark",
		description:
			"Challenge yourself to beat the mark in this exciting individual competition.",
		image: img11,
		formLink: "https://forms.gle/iig8GrqMD6st9wZh9",
		rulebook: "",
		category: "upcoming",
		price: { pool: "Upto 1k", first: "1,000", second: "NA" },
		entryFees: "50/-",
		memberCount: "individual",
		date: "24/08/2024",
		organizers: [
			{
				name: "Akash Kuware",
				email: "info.akash@gmail.com",
				phone: "+91 9322881266",
			},
			{
				name: "Devanshu Soni",
				email: "info.devanshu@gmail.com",
				phone: "+91 6305229849",
			},
		],
		coOrganizers: [
			{
				name: "Kaustubh Kale",
				email: "info.kaustubh@gmail.com",
				phone: "+91 XXXXX XXXXX",
			},
			{
				name: "Janvi Bhaise",
				email: "info.janvi@gmail.com",
				phone: "+91 XXXXX XXXXX",
			},
		],
	},
	{
		id: 13,
		title: "Arm Wrestling",
		description:
			"Show your strength in the Arm Wrestling competition. Who will be the strongest?",
		image: img12,
		formLink: "https://forms.gle/W1m28xrnsrZuAfu68",
		rulebook: "",
		category: "upcoming",
		price: { pool: "Upto 600", first: "600", second: "NA" },
		entryFees: "60/-",
		memberCount: "individual",
		date: "24/08/2024",
		organizers: [
			{
				name: "Ojas Modak",
				email: "info.ojas@gmail.com",
				phone: "+91 7620186476",
			},
			{
				name: "Umesh Dongre",
				email: "info.ojas@gmail.com",
				phone: "+91 8669338207",
			},
		],
		coOrganizers: [
			{
				name: "Sujal Burande",
				email: "info.sujal@gmail.com",
				phone: "+91 XXXXX XXXXX",
			},
			{
				name: "Laleet Atkare",
				email: "info.laleet@gmail.com",
				phone: "+91 XXXXX XXXXX",
			},
		],
	},
	{
		id: 14,
		title: "Mystic Rhythm",
		description:
			"Feel the beat and move to the rhythm in this exhilarating group event.",
		image: img13,
		formLink: "https://forms.gle/V5qf1R3NRXzhcNQe7",
		rulebook: "",
		category: "upcoming",
		price: {
			pool: "Upto 1k",
			first: "700(dancing) and 700(singing)",
			second: "NA",
		},
		entryFees: "60/- per member",
		memberCount: "Max 4 members",
		date: "24/08/2024",
		organizers: [
			{
				name: "Aditya Jadhav",
				email: "info.aditya@gmail.com",
				phone: "+91 7028373062",
			},
		],
		coOrganizers: [
			{
				name: "Prutha Raut",
				email: "info.prutha@gmail.com",
				phone: "+91 XXXXX XXXXX",
			},
			{
				name: "Khushi Nasre",
				email: "info.khushi@gmail.com",
				phone: "+91 XXXXX XXXXX",
			},
		],
	},
	{
		id: 15,
		title: "Valorant",
		description:
			"Gear up for intense tactical action in the Valorant team-based competition.",
		image: img14,
		formLink: "https://forms.gle/CyZB4Wcd9ViR173s6",
		rulebook: "",
		category: "upcoming",
		price: { pool: "upto 5k", first: "NA", second: "NA" },
		entryFees: "60/- per person",
		memberCount: "Team of 5",
		date: "24/08/2024 and 25/08/2024",
		organizers: [
			{
				name: "Sajesh Atkare",
				email: "info.sajesh@gmail.com",
				phone: "+91 9860321398",
			},
			{
				name: "Tushar Sakharkar",
				email: "info.tushar@gmail.com",
				phone: "+91 9699154886",
			},
		],
		coOrganizers: [
			{
				name: "Sarvesh Nashikwar",
				email: "info.sarvesh@gmail.com",
				phone: "+91 XXXXX XXXXX",
			},
			{
				name: "Harsh Naidu",
				email: "info.harsh@gmail.com",
				phone: "+91 XXXXX XXXXX",
			},
		],
	},
];


export default eventData;
